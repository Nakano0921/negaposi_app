...
error_flg = False
# 一休レストランのTOPページ
url = 'https://restaurant.ikyu.com/?visit_date=2020-12-10&visit_time=19%3A00&expand_visit_minutes=60&num_guests=2'
driver.get(url)
sleep(1)
# 「検索する」のボタンをクリック
...
if error_flg is False:
  try:
    ...
    pass
assesments2 = driver.find_elements_by_class_name('des_gdIDUsrImprBox')
for assesment2 in assesments2:
  if i == 24:
    break
  comment = assesment2.find_element_by_xpath(f'//*[@id="des_inner"]/div[{i}]/div[3]/table/tbody/tr[3]/td').text
  comments.append(comment)
  taste = assesment2.find_element_by_xpath(f'//*[@id="des_inner"]/div[{i}]/div[1]/table/tbody/tr[2]/td[2]/span').text
  tastes.append(taste)
  service = assesment2.find_element_by_xpath(f'//*[@id="des_inner"]/div[{i}]/div[1]/table/tbody/tr[3]/td[2]/span').text
  services.append(service)
  i +=2
...
df[taste] = tastes
df[service] = services
df.to_csv('testment.csv', index=False)